...
| LicenseGate (P1 enforcement) | ✅ Implemented, verified clean against this repo's own 135 dependencies (allow-list extended for two real-world license-string variants found along the way: `multidict`'s bare `"Apache License 2.0"` field and `matrix-nio`'s full-LICENSE-text `"Internet Systems Consortium"` field) |
...
| `BreachAlert`/72-hour DPDP notification clock, `AuditChainBreachDetector` (tamper -> alert), `VaultAccessMonitor` (rate-threshold anomaly detection) (Epic 3.3) | ✅ Implemented and tested — the two signals this codebase can raise mechanically (audit-chain tamper, vault-access rate) without an external anomaly model; the downstream incident-response Langflow flow/PDF dossier and the real Grafana/Prometheus alert rules aren't built here |
| Indic STT/TTS voices (IndicConformer/IndicWhisper, Piper ta-IN/hi-IN), Consent Manager integration | ⬜ Phase 3 (remaining) |
...
| `MatrixWhatsAppAdapter` (Epic 4.5) | ✅ Implemented against the real, installed `matrix-nio` client (ISC-licensed) — verified `AsyncClient.room_send`/`add_event_callback` signatures, tests use genuine `nio.RoomMessageText.from_dict(...)` events — but never exercised against a live Matrix homeserver/mautrix-whatsapp bridge (no network path to one in this sandbox) |
...
| `UsageEventSink` contract + `LagoUsageEventSink` (Epic 4.4) | ✅ Implemented against the real, installed `lago-python-client` (MIT-verified) — its `EventClient.create`/`Event` model signatures, including a real quirk this caught (its `timestamp` field's `Union[str, int, None]` ordering always coerces to `str` under pydantic v1) — but never exercised against a live Lago instance (no network path to one here); `register()` raises rather than guessing at an API key/URL |
...
| ICD-10 vertical PII recognizer (Epic 5.1): `SimplePIIAnalyzer`/`PresidioAnalyzer` now take an optional `vertical=` to opt a dental/healthcare tenant's diagnosis codes into masking, additive to the core Indian-market recognizers | ✅ Implemented and tested — another docstring-referenced-but-missing piece (`saap/compliance/pii.py`'s module docstring has said "plus vertical packs (ICD codes...)" since Phase 0) |
| `render_opportunity_matrix_pdf` (AI Audit flow's Report Writer, Epic 5.4) | ✅ Implemented via `reportlab` (BSD-3-Clause); tests genuinely parse the rendered PDF back with `pypdf` (dev-only dep) and assert the computed figures appear in the extracted text, not just "didn't crash" |
| Remaining vertical packs (dental/healthcare, legal/e-commerce), web-scrape + LLM-driven Acquisition-Engine/Report-Writer agent steps | ⬜ Phase 5 (remaining) — largely Langflow flow JSON authoring rather than Python, and this codebase can't install/verify against the real `langflow` package (see note below) |
| `build_evidence_pack` (Phase 6, DPDP Phase-3 evidence pack): bundles erasure certificates, breach alerts, and a fresh audit-chain integrity check per tenant | ✅ Implemented and tested, composing already-tested real components (`ErasureService`, `BreachAlert`, `AuditStore.verify_chain`) — encryption attestations and processor contracts are deployment/legal artifacts this codebase has no way to generate, so the pack leaves those fields for the caller to attach |
| Helm/`scale`/`edge` deployment profiles, chaos testing, fine-tuning (Unsloth/PEFT/Axolotl) | ⬜ Phase 6 (remaining) — needs a real cluster/GPU fleet to test against, nothing to unit-test without one |
